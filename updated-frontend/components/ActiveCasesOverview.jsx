...
export default function ActiveCasesOverview({ patients }) {
  ...
  const statusCounts = {
    awaiting_doctor: patients.filter((p) => p.status === "awaiting_doctor")
      .length,
    video_scheduled: patients.filter((p) => p.status === "video_scheduled")
      .length,
    completed: completedCases.length,
  };

  const priorityCounts = {
    High: patients.filter((p) => extractPriority(p) === "High").length,
    Medium: patients.filter((p) => extractPriority(p) === "Medium").length,
    Low: patients.filter((p) => extractPriority(p) === "Low").length,
  };

  const completionRate =
    patients.length > 0 ? (completedCases.length / patients.length) * 100 : 0;
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600">
          Monitor your current patient cases and their statuses
        </p>
      </div>
            ...
            {priorityCounts.High > 0 && (
              <div className="pt-4 border-t">
                <div className="flex items-center space-x-2 text-red-600">
                  ...
                  <span className="text-sm font-medium">
                    {priorityCounts.High} high priority case
                    {priorityCounts.High > 1 ? "s" : ""} need attention
                  </span>
                </div>
              </div>
            )}
